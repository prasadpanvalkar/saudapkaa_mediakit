...
import {
    ...
    Phone, Video, Ruler, DollarSign, Shield, TrendingUp, User
} from "lucide-react";
...
export default function EditPropertyPage() {
    ...
    useEffect(() => {
        ...
        const userRole = Cookies.get("user_role"); // Adjust based on how you store role
        ...
        // Check if user is admin - Basic check, backend enforces real security
        // Assuming 'is_superuser' or similar role flag is stored, or just rely on backend 403
        // For now, we'll allow access if token exists, but backend will block non-admins
        // ideally: if (userRole !== "admin") router.push("/dashboard");
    ...
    const fetchProperty = async () => {
        ...
        try {
            ...
            setAdminFields({
                ...
                is_verified: property.is_verified === true,
                priority_listing: property.priority_listing === true,
                admin_notes: safeGet(property.admin_notes, ""),
                owner_email: property.owner?.email || property.owner_email || "",
            });

            console.log("Admin fields loaded:", {
                is_featured: property.is_featured,
                is_verified: property.is_verified,
                priority_listing: property.priority_listing,
                has_admin_notes: !!property.admin_notes
            });
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            // Append Admin Fields
            if (adminFields.is_featured) data.append("is_featured", "true");
            else data.append("is_featured", "false");

            if (adminFields.is_verified) data.append("is_verified", "true");
            else data.append("is_verified", "false"); // Or maybe don't send if false depending on backend, but explicit is safer

            if (adminFields.priority_listing) data.append("priority_listing", "true");
            else data.append("priority_listing", "false");

            if (adminFields.admin_notes.trim()) data.append("admin_notes", adminFields.admin_notes);
            if (adminFields.owner_email.trim()) data.append("owner_email", adminFields.owner_email);
    ...
    return (
        <div className="min-h-screen bg-gray-50 flex flex-col">
                ...
                <div className="max-w-7xl mx-auto px-4 py-4">
                    <div className="flex items-center justify-between mb-3">
                        <Link href="/admin/properties" className="flex items-center text-gray-600 hover:text-gray-900 transition-colors">
                            ...
                            <span className="font-medium">Back to Admin Properties</span>
                        </Link>
                        <div className="text-center">
                            <span className="text-sm font-bold block text-gray-900">Admin: Editing Property</span>
                            <span className="text-xs text-gray-500">ID: {propertyId}</span>
                        </div>
                ...
                <div className="bg-gradient-to-r from-blue-50 to-green-50 border-l-4 border-blue-500 p-6 rounded-lg mb-6 shadow-sm">
                    <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1">
                            <h3 className="text-lg font-bold text-gray-900 mb-2">Admin: How to Edit Property</h3>
                            <ul className="space-y-1 text-sm text-gray-700">
                                <li className="flex items-start gap-2">
                                    <Shield className="w-4 h-4 text-purple-600 flex-shrink-0 mt-0.5" />
                                    <span>You are editing as an administrator - you can modify ANY property</span>
                                </li>
                                <li className="flex items-start gap-2">
                                    <CheckCircle className="w-4 h-4 text-green-600 flex-shrink-0 mt-0.5" />
                                    <span>All existing data has been pre-filled across all 6 steps</span>
                                </li>
                                <li className="flex items-start gap-2">
                                    <CheckCircle className="w-4 h-4 text-green-600 flex-shrink-0 mt-0.5" />
                                ...
                                <li className="flex items-start gap-2">
                                    ...
                                    <span>Step 6 includes admin-only settings (Featured, Verified, Priority, Owner Assignment)</span>
                                </li>
                            </ul>
                        </div>
                    </div>
                    ...
                    >
                        ...
                        {step === 6 && (
                            ...
                            >
                                ...
                                {/* 0. ADMIN SETTINGS (ADMIN ONLY) */}
                                <div className="bg-purple-50 p-6 sm:p-8 rounded-2xl shadow-sm border border-purple-200">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        ...
                                        <div className="space-y-4">
                                            <div className="flex items-center justify-between p-3 bg-white rounded-lg border border-purple-100">
                                                <div>
                                                    <span className="font-semibold text-gray-900 block">Featured Listing</span>
                                                    <span className="text-xs text-gray-500">Show in featured sections</span>
                                                </div>
                                                <input
                                                    type="checkbox"
                                                    checked={adminFields.is_featured}
                                                    onChange={(e) => setAdminFields(p => ({ ...p, is_featured: e.target.checked }))}
                                            ...
                                            <div className="flex items-center justify-between p-3 bg-white rounded-lg border border-purple-100">
                                                <div>
                                                    <span className="font-semibold text-gray-900 block">Verified Property</span>
                                                    <span className="text-xs text-gray-500">Mark with verification badge</span>
                                                </div>
                                                <input
                                                    type="checkbox"
                                                    checked={adminFields.is_verified}
                                                    onChange={(e) => setAdminFields(p => ({ ...p, is_verified: e.target.checked }))}
                                                    className="w-5 h-5 accent-purple-600"
                                                />
                                            </div>
                                            ...
                                            <div className="flex items-center justify-between p-3 bg-white rounded-lg border border-purple-100">
                                                <div>
                                                    <span className="font-semibold text-gray-900 block">Priority Listing</span>
                                                    <span className="text-xs text-gray-500">Boost ranking in searches</span>
                                                </div>
                                                <input
                                                    type="checkbox"
                                                    checked={adminFields.priority_listing}
                                                    onChange={(e) => setAdminFields(p => ({ ...p, priority_listing: e.target.checked }))}
                                                    className="w-5 h-5 accent-purple-600"
                                                />
                                            </div>
                                        </div>
                                        ...
                                        <div className="space-y-4">
                                            <div>
                                                <Label className="text-xs font-bold uppercase text-purple-800 mb-1 block">Owner Email (Reassign)</Label>
                                                <Input
                                                    value={adminFields.owner_email}
                                                    onChange={(e) => setAdminFields(p => ({ ...p, owner_email: e.target.value }))}
                                                    placeholder="owner@example.com"
                                                    className="bg-white border-purple-200 focus:ring-purple-500"
                                                />
                                            </div>
                                            <div>
                                                <Label className="text-xs font-bold uppercase text-purple-800 mb-1 block">Admin Notes (Internal)</Label>
                                                <Textarea
                                                    value={adminFields.admin_notes}
                                                    onChange={(e) => setAdminFields(p => ({ ...p, admin_notes: e.target.value }))}
                                                    placeholder="Notes visible only to admins..."
                                                    className="bg-white border-purple-200 focus:ring-purple-500 min-h-[80px]"
                                                />
                                            </div>
                                        </div>
                                    </div>
            ...
            {/* Footer Actions */}
            <div className="bg-white border-t p-4 fixed bottom-0 right-0 left-0 lg:left-72 z-40">
